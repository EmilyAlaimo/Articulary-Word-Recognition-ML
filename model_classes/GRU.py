...
class GRUClassifier(torch.nn.Module):
    ...
    def forward(self, x):                               # x: (Batch size (32), lunghezza sequenza (144), input_dim (9))
        if self.bidirectional:
            _, h_n = self.gru(x)                        # h_n: (2*num_layers (per ogni layer due direzioni), 32, 128)
            h = torch.cat([h_n[-2], h_n[-1]], 1)        # h: (32, 2*128) stato finale -> h_n[-2] e h_n[-1] stati finali (da sinistra verso destra e da destra verso sinistra)
        else:
            _, h_n = self.gru(x)                        # h_n: (num_layers, 32, 128)
            h = h_n[-1]                                 # (32, 128) stato finale (solo 1 nel caso unidirezionale)
